...
model.loader = function(input) {
  ...
  P = read.table(input, skip = 3, nrows = m, sep = " ", header = FALSE)
  Q = read.table(input, skip = 3+m, nrows = n, sep = " ", header = FALSE)
  P = P[,-c(1,ncol(P))]
  Q = Q[,-c(1,ncol(Q))]
  P = data.matrix(P)
  Q = data.matrix(Q)
...
libmf = function(triplet, m, n, k = 10, cost = 0.1, lrate = 0.1, niter = 20, 
...
#   triplet[,2] = triplet[,2] - min(triplet[,2])
  ...
  r = Reco()
  r$train(trainfile.temp, opts = list(dim = k, cost = cost, lrate = lrate, niter = niter,
...
libmf.tune = function(triplet, m, n, k = 10, nfold = 10, cost = 0.1, lrate = 0.1, niter = 20, 
  ...
  write.table(triplet, file = trainfile.temp, sep = " ", quote = FALSE, 
              row.names = FALSE, col.names = FALSE)
  r = Reco()
  res = r$tune(trainfile.temp, opts = list(dim = k, cost = cost, lrate = lrate, niter = niter,
                                           nthread = nthread, nmf = nmf, verbose = verbose, 
                                           nfold = nfold))
  return(res)
}
