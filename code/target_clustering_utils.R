...
get_target_clusters = function(dataset, target_sim_mat, thresh){
  sim_mat = target_sim_mat
  set = c(1:nrow(target_sim_mat))
  already_clustered = c()
  cluster_list = list()
  n_clusters = 0
  ...
  while(length(set)>0){
    ...
    if (nn>0){
      ...
      return(list(cluster_list, set))
    }
    
    already_clustered = union(already_clustered, cluster)
    cat('training data of cluster: ',length(which(dataset[,2]%in%cluster)),'\n')
