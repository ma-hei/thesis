...
inds = sample(1:nrow(dataset_triplets), floor(nrow(dataset_triplets)/2))
dataset_triplets_temp = dataset_triplets[inds,]

while(min(table(dataset_triplets_temp[,1]))<2 || min(table(dataset_triplets_temp[,2]))<2){
...
dataset_triplets = dataset_triplets_temp
...
save(test_folds, dataset_triplets, drug_adj_mat, target_adj_mat, file = "5fold_cv_data_kiba_sparse.Rda")
