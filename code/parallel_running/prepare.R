...
target_adj_mat = make_adj_mat_thresh(sim_mat = target_sim, thresh = 0.7, FALSE)
drug_adj_mat = make_adj_mat_thresh(sim_mat = drug_sim, thresh = 0.75, FALSE)

n_folds = 5
test_folds = get_folds(dataset_triplets, n_folds)

save(test_folds, dataset_triplets, drug_adj_mat, target_adj_mat, file = "5cv_data.Rda")
