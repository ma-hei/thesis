...
target_adj_mat_below[which(target_adj_mat>=0.6)] = 0
target_adj_mat_above = target_adj_mat
target_adj_mat_above[which(target_adj_mat<0.6)] = 0


drug_adj_mat_below = drug_adj_mat
drug_adj_mat_below[which(drug_adj_mat>=0.6)] = 0
drug_adj_mat_above = drug_adj_mat
drug_adj_mat_above[which(drug_adj_mat<0.6)] = 0
...
for (fold in 1:n_folds){
  ...
  #for (d in from:to){
    ...
    if (length(drug_set)>1){
      ...
      training_adj_mats_1 = make_training_adj_mat(train_mat = train_mat[drug_set,], drug_adj_mat = drug_adj_mat_below, target_adj_mat = target_adj_mat_below, drug_set = drug_set, target_set = c(1:n_targets))
      
      training_adj_mats_2 = make_training_adj_mat(train_mat = train_mat[drug_set,], drug_adj_mat = drug_adj_mat_above, target_adj_mat = target_adj_mat_above, drug_set = drug_set, target_set = c(1:n_targets))

      adj_mats = list()
      adj_mats[[1]] = training_adj_mats_1[[1]]
      adj_mats[[2]] = training_adj_mats_1[[2]]
      adj_mats[[3]] = training_adj_mats_2[[3]]
      adj_mats[[4]] = training_adj_mats_2[[4]]

      eta = 0.001
      
      params = train_crf(training_data = train_data, adj_mats = adj_mats, X = X, crf_iters = 200, eta = eta, 1)
      ...
      prediction_adj_mats_1 = make_prediction_adj_mat(train_mat = train_mat[drug_set,], drug_adj_mat = drug_adj_mat_below, target_adj_mat = target_adj_mat_below, drug_set = drug_set, target_set = c(1:n_targets))
      prediction_adj_mats_2 = make_prediction_adj_mat(train_mat = train_mat[drug_set,], drug_adj_mat = drug_adj_mat_above, target_adj_mat = target_adj_mat_above, drug_set = drug_set, target_set = c(1:n_targets))

      adj_mats = list()
      adj_mats[[1]] = prediction_adj_mats_1[[1]]
      adj_mats[[2]] = prediction_adj_mats_1[[2]]
      adj_mats[[3]] = prediction_adj_mats_2[[3]]
      adj_mats[[4]] = prediction_adj_mats_2[[4]]
